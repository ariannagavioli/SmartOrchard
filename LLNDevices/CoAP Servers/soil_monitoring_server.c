...
PROCESS(soil_monitor_server, "Soil Monitor CoAP Server");
AUTOSTART_PROCESSES(&soil_monitor_server);

extern coap_resource_t res_soil;

static int observing_interval = 6;
static int configuration_interval = 0.5;
...
PROCESS_THREAD(soil_monitor_server, ev, data){
	...
	sprintf(msg, "{\"Functionality\":\"soil_monitor\",\"ID\":%d,\"IP\":\"%s\"}",node_id,ip_address);
